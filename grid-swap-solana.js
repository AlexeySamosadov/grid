...
const {
    ...
    COMMISSION_RESERVE // Резерв для комиссии
} = process.env;

const STATE_PATH = path.resolve('grid_state.json');

// Функции для загрузки/сохранения состояния грида с учётом старых флагов
...
async function main() {
    ...
    // Комиссия для 30 ячеек
    const reserve = BigInt(COMMISSION_RESERVE * 1e9); // Умножаем на 1e9 для работы с лямпордами
    ...
    async function trySwap() {
        ...
        try {
            ...
            // Шаг 2: Портфель и свободный капитал
            ...
            const perGridLamports = remain > 0 && freeValueSOL > 0
                ? BigInt(Math.floor((freeValueSOL) * 1e9 / remain))
                : 0n;
